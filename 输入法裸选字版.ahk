...
if(显示字体=="华文细黑"){
	选字窗分割符:="--------------------------------------------------------------------`n"
	初始高度:=80, 选字窗高度:=160, 选字窗宽度:=520
}else if(显示字体=="微软雅黑"){
	选字窗分割符:="----------------------------------------------------`n"
	初始高度:=100, 选字窗高度:=190, 选字窗宽度:=520
}else{
	选字窗分割符:="----------------------------------------------`n"
	初始高度:=100, 选字窗高度:=160, 选字窗宽度:=540
}
...
英文括号按键:={"+9":"(){left}", "+,":"<>{left}",  "[":"[]{left}"			;简化括号的发送问题
		,  "+[":"{{}{}}{left}", "+'":"""""{left}"}
引号已发送:=1								;解决双引号问题
默认句子:=已确认文字:= 插入字符:= 待转化字符 := 句末标点 :=词典字串:=""
字母键到词表 :={}, 数字键到词表 :={}
上翻页键 := "-[,", 下翻页键 := "=]."
...
中文输入(按键){
	...
	if (Strlen(插入字符)==0 && strLen(待转化字符)==0 && !instr(字母选字键,按键) && 按键!="+'"){		;处理非打字状态下的特殊按键，包括符号，排除引号
		...
		if(插入状态){
			插入字符.=按键,	词典字串:=产生字串(插入字符)
		}else	待转化字符 .= 按键, 词典字串:=产生字串(待转化字符)
		产生候选()
		...
		if(选中字){
			...
			if(strlen(插入字符)>0){
				...
				if(strlen(插入字符)==0){
					插入状态:=0, 词典字串:=产生字串(待转化字符)		;插入选字状态结束，继续转化剩余字串
				}else 词典字串:=产生字串(插入字符)
			}else{
				待转化字符:=substr(待转化字符,strlen(选中字)*2+1), 词典字串:=产生字串(待转化字符)
			}
			gosub,判断发送上屏
		}else if(!插入字符){
...
产生字串(原字符){
	global 词频拼音表1,词频拼音表2,双拼编码
	待转化字符:=双拼转换(原字符, 双拼编码)
	返回字串:=""
	if(strlen(待转化字符)==1){
		if(词频拼音表1[待转化字符]){
			返回字串 .= 词频拼音表1[待转化字符]			
		}else 返回字串 .= 词频拼音表2[待转化字符]
		return 返回字串
	}
...
;构造字母选字项	
	...
	for 序号,值 in 字母选字键组
	{
		字母键到词表[值]:=词频词组[序号+strlen(数字选字键)+翻页数*(strlen(数字选字键)+strlen(字母选字键))]	;字母做键,字做值
		字 :=字母键到词表[值]
		显示候选字串 .= 值 . "."
		if (!字){
			显示候选字串 .= "　"
...
显示候选框:
	...
	if(需重建窗口){ 						
		...
		SplashImage,,x%窗口x% y%窗口y% b1 h%初始高度% w%选字窗宽度% c10 fm14 fs14 wm400 ws400
...
生成默认句子(字符串){
	默认句子:=""
	if(mod(strlen(字符串),2)==1){
		字符串:=substr(字符串,1,-1)
	}
	while (strlen(字符串)>=2){
		字串组:=strsplit(产生字串(字符串),",")
		当前词:=字串组[1]
		默认句子.=当前词
		if(当前词){
			字符串:=substr(字符串,strlen(当前词)*2+1)
		}else break						;小心无限循环!!!
	}
	return 默认句子
}
