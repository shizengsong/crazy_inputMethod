...
英文括号按键:={"+9":"(){left}", "+,":"<>{left}",  "[":"[]{left}"				;简化括号的发送问题，发送对称括号
		,  "+[":"{{}{}}{left}", "+'":"""""{left}"}
引号已发送:=1									;解决双引号问题
汉语结构单字:="自从到往在由向于至趁当按靠照用据拿比因为被给让叫归把将管对跟和给替同和跟与及或而并的得地着了过连们所吗么吧呢啊着嘛呗啦嘞喽都全单共光尽净仅就又只才可倒却不没别刚正将老总很极最太更"						;汉语里面常用的结构性的单字
待保存词语:=已确认文字:= 插入字符:= 构造词记忆:=用户自造词拼音:=已转化字符:=待转化字符 := 原字符:= 句末标点 :=gb词典字串 :=常用词典字串:=""
用户词典表:={}, 字母键到字表 :={}, 数字键到词表 :={}
上翻页键 := "-[,", 下翻页键 := "=]."
前选字为单字:=翻页数:=选字状态:=0
所有按键:=["q","w","e","r","t","y","u","i","o","p","a","s","d","f","g","h"			;所有快捷键(除带shift的标点,及^+空格)
		,"j","k","l","z","x","c","v","b","n","m", "1","2","3","4"
		,"5","6","7","8","9","0",";", ",", ".", "/","=","-"
		,"esc","backspace","space","enter","tab","delete","f4"]
...
盲打出字:
	...
	位置:=instr(字母选字键,substr(待转化字符,3,1))
	send,% gb词典词组[位置]
	输入置空()
return
