...
;非单字模式下,在输入三个字符的情况下,按空格键可以盲打出字(例如dee加空格，将打出“的”字，ler加空格，会打出“了”字)
...
;					翻页数-=1
;				}else 句末标点 := 中文标点["" . 按键]
;			}else		; if(!instr(下翻页键,按键)&&!instr(上翻页键,按键) )
			选字状态:=1
			gosub,判断发送上屏
			return
		}
		选字状态:=1
		if(按键=="tab"){
		}else if(按键=="space") {						;在三个字符的情况下,盲打出字
...
显示候选框:
	...
	if(需重建窗口){ 						
		...
		if(strlen(较常用字)>11){
			宽度:=(strlen(较常用字)-1-10)*4*4+选字窗宽度
		}else 宽度:=选字窗宽度
			
		if(选字状态){
			WinMove, 疯狂输入法选字框, , , ,%宽度%,%选字窗高度%,	;宽度不规范，等下再调
		}else{
			WinMove, 疯狂输入法选字框, , , ,%宽度%
		}
