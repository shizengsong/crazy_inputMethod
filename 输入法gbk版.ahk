...
if(!单字模式){
	...
	词频拼音表2 :=生成音词表(".\词典\正常词典2.txt")			;拼音词典,来自影子输入法(ahk关联数组超过24万,载入好像就变慢,所以切分成两个词典)
...
~Lshift::
keywait,shift,t0.25							;用此句修正和其他的按键冲突
if errorlevel
	return
获取光标位置()
...
if(输入法开关){
	中文输入(按键)
}else if (按键=="space"||按键=="backspace"||按键=="enter" ||按键=="esc"||按键=="tab"||按键=="delete" ){
	send,{%按键%}
...
中文输入(按键){
	...
	if (字符数==0 && !instr(字母选字键,按键) && 按键!="+'"){		;处理非打字状态下的特殊按键，包括符号，排除引号
		...
		if(中文标点[按键]){
			if(选字状态){	
				if (instr(下翻页键,按键)){			;若标点为句号或逗号，则进行翻页
					翻页数+=1
...
删除操作:
	if (strlen(已确认文字)>0){						;可能是插入状态
		if(strlen(插入字符)>0){
			...
			if(按键=="backspace"){
				已确认文字:=substr(已确认文字,1,-1)	;开始进入插入状态
			}else if(按键=="delete")
				待转化字符 := subStr(待转化字符,3)	;删除剩余字符的句首字母
			选字状态:=0, 插入状态:=1
		}
	}else {								;输入状态的删除
...
产生字串(原字符){
	global 常用词典字串,gb词典字串,词频拼音表1,词频拼音表2,gbk单字表,双拼编码
	待转化字符:=双拼转换(原字符, 双拼编码)
	常用词典字串:=""
...
产生候选(){
	...
	for 序号,值 in 字母选字键组
		字母键到字表[值]:=gb词典词组[序号+翻页数*strlen(字母选字键)]
	for 序号,值 in 数字选字键组{
		字:=数字键到词表[值] :=词频词组[序号+翻页数*strlen(数字选字键)]		;数字做键,词做值
		if(选字状态){
			for k,字母 in 字母选字键组{
				if (字母键到字表[字母]==字)
					值:=字母,break					;检测和字母选区重合的字
			}
		}		
		显示候选字串 .= 值 . "." 
		if (!字){
...
显示候选框:
	...
	if(需重建窗口){ 						
		...
		SplashImage,,x%窗口x% y%窗口y% b1 h%选字窗高度% w%选字窗宽度% c10 fm14 fs14 wm400 ws400
