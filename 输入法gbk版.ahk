...
中文输入(按键){
	...
	if (字符数==0 && !instr(字母选字键,按键) && 按键!="+'"){		;处理非打字状态下的特殊按键，包括符号，排除引号
		...
		if(中文标点[按键]){
			if(句末标点!=""){	
;				msgbox,% 中文标点[按键]					;若标点为句号或逗号,可进行翻页
				if (instr(下翻页键,按键)){
					翻页数+=1
...
产生字串(临时字符){
	global 常用词典字串,gb词典字串,词频拼音表,gbk单字表
	常用词典字串:=""
	gb词典字串 := gbk单字表[substr(临时字符,1,2)]
	while (strlen(临时字符)>=1){
		if(strlen(临时字符)==1){
			常用词典字串 .= 词频拼音表[临时字符]
			临时字符 := ""
		}else if(strlen(临时字符)>=2){
			if(mod(strlen(临时字符),2)==0){
				常用词典字串 .= 词频拼音表[临时字符]
				临时字符:=substr(临时字符,1,-2)
			}else{
...
产生候选(){
	local 显示输入字串,显示候选字串,词频词组,数字选字键组,字母选字键组
	显示输入字串 .= 已确认文字 . 插入字符
	if(插入状态){
		显示输入字串 .="_  " . 待转化字符 . 句末标点
