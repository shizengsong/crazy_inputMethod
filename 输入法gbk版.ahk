...
#include 程序头文件.ahk		;程序头文件,通常不用修改

;请把默认使用英文标点的程序写到这里
使用英文标点的程序列表:="emeditor.exe,scite.exe,notepad.exe,explorer.exe,cmd.exe"	
;需要中文输入的程序列表
...
启用中文标点:=1									;这个不用改,已经是自动的了,这个只是初始化时的标点
...
待保存词语:=已确认文字:= 插入字符:= 构造词记忆:=用户自造词拼音:=已转化字符:=待转化字符 := 原字符:= 句末标点 :=gb词典字串 :=常用词典字串:=""
用户词典表:={}, 字母键到字表 :={}, 数字键到词表 :={}
上翻页键 := "-[,", 下翻页键 := "=]."
前选字为单字:=翻页数:=选字状态:=0
...
~Lshift::
keywait,shift,T0.5								;用此句修正和其他的按键的冲突
...
;					句末标点 := 中文标点["" . 按键]
		...
		if(选中词){
			...
			if(strlen(插入字符)>0){
				...
				已转化字符.=substr(待转化字符,1,strlen(选中词)*2)
		...
		if(插入状态){
			...
			原字符.=按键
			if(mod(strlen(待转化字符),2)==1){
				末尾字符:=substr(待转化字符,0)				;纠正错误的键序,支持简单的并击
				if(!gbk单字表[双拼转换(末尾字符 . 按键, 双拼编码)]){	;如果拼不出字,则调换声韵母位置
					待转化字符:=substr(待转化字符,1,-1) . 按键 .  末尾字符
				}else 待转化字符 .= 按键
			}else 待转化字符 .= 按键 
			产生字串(待转化字符)
		}
	...
	if(插入状态){	
		...
		if(按键=="backspace"){						;刚进入插入状态
			if(strlen(已确认文字)<=2){				;已确认文字较短情况下
				待转化字符:=已转化字符 . 待转化字符,原字符:=已转化字符 . 原字符		;有一些问题,导致原字符显示不对.这里原字符?
				已确认文字:="", 已转化字符:=""
			}else if(!数字键到词表[1]){				;拼不出字的情况下
				if(strlen(待转化字符)!=1){
					待转化字符:=substr(待转化字符,1,1),原字符:=substr(原字符,1,1)
				}else 待转化字符:="",原字符:=""
			}else 插入状态:=1					;刚进入插入状态，不删字
...
输入置空(){
	...
	已转化字符:=待保存词语:=用户自造词拼音:=用户自造词:=gb词典字串 :=已确认文字:=插入字符 := 原字符:=待转化字符 := 句末标点 :=""
	前选字为单字:=插入状态:=选字状态:=翻页数:=0
	字母键到字表 :={}
}
发送中文标点(按键){
	global 引号已发送,中文标点
...
产生候选(){
	...
	for 序号,值 in 数字按键组{
		字:=数字键到词表[值] :=常用词典词组[序号+翻页数*strlen(数字按键)]	;数字做键,词做值
		显示输入字串 .= 值 . "." 
		if (!字){
			显示输入字串 .= "　"
		}else 显示输入字串 .= 字 
	...
	for 序号,值 in 字母按键组
	{
		字母键到字表[值]:=gb词典词组[序号+翻页数*strlen(字母按键)]		;字母做键,字做值
		字 :=字母键到字表[值]
		if(字 && instr(较常用字,字)){
			显示候选字串 .= "♦" . 值 
		}else 显示候选字串 .= 值 . "."
...
#if winactive(A_ScriptFullPath)					;在脚本保存后重启脚本
~^s::reload
