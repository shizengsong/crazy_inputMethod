...
;双拼编码, 设置文件在双拼编码文件夹下,若没有则到双拼编码文件夹中去建
...
~Lshift::
keywait,shift,t0.25							;修正和其他的按键冲突
if errorlevel
	return
获取光标位置()
...
显示候选框:
	...
	if(需重建窗口){ 						
		...
		SplashImage,,x%窗口x% y%窗口y% b1 h%选字窗高度% w%选字窗宽度% c10 fm14 fs14 wm400 ws400,%显示候选字串%,%显示输入字串%,疯狂输入法选字框,%显示字体%
		需重建窗口:=0
	}else{ 
