...
menu,tray,icon,%A_ScriptDir%\图标\图标文件.icl,1				;拖盘图标,提取自影子输入法,觉得图标字体很不错,借用一下,不好意思
...
表情:={"haha":"O(∩_∩)O","hehe":"^_^ ","oe":"-_-#"				;表情功能,请自行添加(自己定义字符,可自行修改)
	,"hj":"-_-!","of":":-) ","hkde":"-)","yn":"@_@","xixi":"^_^"}		;hj:汗，of:嗯 等均为按照智能abc的双拼编码，不需要请自行修改
...
if(进程名&&instr(使用英文标点的程序列表,进程名)){
	临时启用中文标点:=1
	settimer,倒计时进入英文标点,-5000					;临时中文标点的时间,默认5秒,可自行修改
...
中文输入(按键){									;输入法核心步骤
	...
	if (字符数==0 && !instr(字母选字键,按键)){				;处理非打字状态下的特殊按键，包括符号，排除引号
		...
		if(选中字){
			...
			}else 待转化字符:=substr(待转化字符,3), 产生字串(待转化字符)
			gosub,判断发送上屏
		}else if(!插入字符){
...
删除操作:
	if(插入状态){	
		...
		if(按键=="backspace"){						;刚进入插入状态
			...
			}else 插入状态:=1					;刚进入插入状态，不删字。已确认文字:=substr(已确认文字,1,-1)
